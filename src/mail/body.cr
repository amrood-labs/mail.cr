module Mail
  ...
  class Body
    ...
    # any tampering
    getter raw_source : String?
    getter parts : PartsList
    ...
    def initialize(string = "")
      ...
      if Utilities.blank?(string)
        ...
        if string.responds_to?(:join)
          @raw_source = Utilities.to_crlf(string.join(""))
        elsif string.responds_to?(:to_s)
          @raw_source = Utilities.to_crlf(string.to_s)
        else
    ...
    #
    #   body = Mail::Body.new('The body')
    #   body.includes?('The') #=> true
    #
    #   body = Mail::Body.new("VGhlIGJvZHk=\n")
    #   body.encoding = 'base64'
    #   body.includes?('The') #=> true
    def includes?(other)
      self.decoded.includes?(other.to_s)
    end
    ...
    def encoded(transfer_encoding = nil)
      if multipart?
        ...
        dec = Encodings.get_encoding(encoding)
        enc =
    ...
    def decoded
      ...
      if !Encodings.defined?(encoding)
        raise UnknownEncodingType.new("Don't know how to decode #{encoding}, please call #encoded and decode it yourself.")
      else
    ...
    def ascii_only?
      @ascii_only = raw_source.to_s.ascii_only?
    end
    ...
    private def crlf_boundary
      "\r\n--#{boundary}\r\n"
    end

    private def end_boundary
      "\r\n--#{boundary}--\r\n"
    end

    private def set_charset
