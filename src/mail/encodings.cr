...
module Mail
  ...
  class UnknownEncodingType < Exception # :nodoc:
  end
  ...
  module Encodings
    ...
    def self.defined?(name)
      @@transfer_encodings.keys.includes? get_name(name)
    end
    ...
    # if RUBY_VERSION < '1.9'
    #   # With KCODE=u we can't use regexps on other encodings. Go ASCII.
    #   def self.with_ascii_kcode #:nodoc:
    #     if $KCODE
    ...
    #  #=> 'This is あ string'
    private def self.b_value_decode(str)
    ...
    #  #=> 'This is あ string'
    private def self.q_value_decode(str)
      RubyVer.q_value_decode(str)
    end

    # private def self.find_encoding(str)
    #   RUBY_VERSION >= '1.9' ? str.encoding : $KCODE
    # end

    # Gets the encoding type (Q or B) from the string.
    private def self.value_encoding_from_string(str)
      str[ENCODED_VALUE, 1]
    end
    ...
    private def self.collapse_adjacent_encodings(str)
      results = [] of String
    ...
    private def self.each_base64_chunk_byterange(str, max_bytesize_per_base64_chunk, &block)
    ...
    # Partition the string into bounded-size chunks without splitting
    # multibyte characters.
    private def self.each_chunk_byterange(str, max_bytesize_per_chunk)
