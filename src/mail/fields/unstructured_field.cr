...
module Mail
  ...
  class UnstructuredField < CommonField # :nodoc:
    def initialize(name, value, charset = "UTF-8")
      if value.is_a?(Array)
        # Probably has arrived here from a failed parse of an AddressList Field
        value = value.join(", ")

        # Mark UTF-8 strings parsed from ASCII-8BIT
      elsif value.responds_to?(:encode) && !value.valid_encoding?
        value = String.new value.encode("UTF-8")
      end

      super value.to_s, charset
    end
    ...
    # TODO: Fix Encodings.encode_decode method and use it here...
    private def do_decode
      Utilities.blank?(value) ? nil : value
    end
    ...
    private def wrapped_value # :nodoc:
    ...
    #  unencoded text.)
    private def wrap_lines(name, folded_lines)
      result = ["#{name}: #{folded_lines.shift}"]
      result.concat(folded_lines)
      result.join("\r\n\s")
    end

    # private def fold(prepend = 0) # :nodoc:
    ...
    #       if charset && word.responds_to?(:encoding)
